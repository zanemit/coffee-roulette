...
def get_meeting_times(bounds, spacing):
    ...
    if not all(i < j for i, j in zip(bounds.flatten(), bounds.flatten()[1:])):
        raise ValueError(
            "Ill-defined bounds! Bounds must be strictly increasing!"
        )
    ...
    if np.all(item_lengths <= 2):
        ...
        raise ValueError(
            "Ill-defined bounds! Bounds must denote full hours [[9,12]] or hours and minutes [[900,1200]] *and* be consistent!"
        )
