...
def get_pairs(cfg_path, 
                conv_starters = True, 
                random_time = True, 
                exclude_nonrandom = None):
    """
    ...
    while not everyone_matched:
        ...
        if os.path.exists(roulette_dir/"crRemovedParticipantDict.p"):
            crRemovedParticipantDict = pickle.load(open(roulette_dir/"crRemovedParticipantDict.p", "rb" ))
        ...
        if random_time:
            ...
            t = dt.date.today() # the day the code is run (Tuesday or Wednesday)
            start_date =  t + dt.timedelta(days = 1)
            day_number = int(7 * weeks) # days until the Tuesday before the next draw
        ...
        for p in range(int(participant_number/2)):
            ...
            if random_time:
                ...
                while (meeting_weekday == 'Saturday' or meeting_weekday == 'Sunday'):
                    random_number_of_days = random.randrange(day_number)
                    meeting_date = start_date + dt.timedelta(random_number_of_days)
            ...
            if conv_starters:
                ...
                all_conv_starters = np.arange(len(open(roulette_dir/"crConversationStarters.txt").readlines( )))
                ...
                if available_conv_starters.shape[0] > 0:
                    ...
                    with open(roulette_dir/"crConversationStarters.txt", "r") as text_file:
            ...
            if crProbMatrix[i[id],j[id]] != 0: # all good - these people have not met yet!
                print(output_str)
    ...
    with open(roulette_dir/"results"/f"{rdate}_crPairings_{rtime}.txt", "w") as f:
